...
| DateOfBirth | DATE | No | Customer's birth date | 1994-10-01 | Type 1 |
...
| LocationKey | INT | Yes | Foreign key to Dim_Location | 1, 25, 100 | Type 2 |
...
- **SCD Join Pattern:** `CustomerID = ? AND TransactionDate BETWEEN StartDate AND ISNULL(EndDate, '9999-12-31')`
...
- 99.7% of locations have State/Region = "Unknown" — only 23 major cities were enriched via ETL.City_Lookup
...
> **Note:** Ranges were redesigned from original spec to eliminate gaps and overlaps across the full Recency × Frequency space. The original design had uncovered zones (e.g., Recency 31–59 with Frequency 0–4) which caused FK violations during Package 5 load.
...
#### ETL Notes
- **Critical:** Date parsing uses `TRY_CONVERT(date, TransactionDate, 103)` — style 103 required because server DATEFORMAT=mdy. Using TRY_CAST would silently drop ~60% of records (days ≥ 13 parsed as NULL).
- **SCD Join:** CustomerKey resolved by matching `(CustomerID, CustLocation)` to `Dim_Customer` — avoids duplicate rows for customers with A→B→A location history.
...
| GrowthRate | DECIMAL(5,2) | Yes | % change vs previous month | -30.00, 0.00, 25.00 | Non-Additive |
...
```
IF RecencyScore ≥ 4 AND FrequencyScore ≥ 4 → Random(4.0, 5.0)
IF RecencyScore ≤ 2 AND FrequencyScore ≤ 2 → Random(1.0, 2.5)
ELSE                                        → Random(2.5, 4.0)
```

**Complaint Flag (Synthetic):**
```
IF GrowthRate < -30% → 70% probability of 1, else 0
```
...
GrowthRate > 5%         → "Moderate Growth"
|GrowthRate| ≤ 5%       → "Stable"
GrowthRate ≥ -20%       → "Moderate Decline"
...
BankingSource.dbo.RawTransactions (~147M records, 20 months)
    ↓ Package 1 (SSIS)
...
**Last Updated:** June 2026
**Status:** ✅ ETL Complete — All packages executed and verified
